...
const documents = computed(() =>
  professors.value.map((professor) => {
    ...
    return {
      ...professor,
      tags,
      tokens: buildSearchTokens(`${professor.name} ${professor.description} ${tags.join(' ')}`)
    };
  })
);
...
const rankedProfessors = computed(() => {
  ...
  return documents.value
    .map((professor) => {
      ...
      const semantic = hasQuery ? semanticSimilarity(queryTokens.value, professor.tokens) : 0;
      const tagBoost = hasQuery ? tagSimilarity(queryTokens.value, professor.tags) : 0;
      ...
      const matched = matchReasons(professor, queryTokens.value);
...
function fuzzyScore(input, professor) {
  ...
  const hits = tokens.map((token) => {
    ...
    return Math.max(...tokenize(normalized).map((candidate) => tokenSimilarity(token, candidate)), 0);
  });
...
function matchedCoverage(queryTokensList, professorTokens) {
  ...
  const hits = queryTokensList.filter((token) => professorSet.has(token) || bestTokenSimilarity(token, professorTokens) >= 0.62).length;
...
function tagSimilarity(queryTokensList, tags) {
  if (!queryTokensList.length || !tags.length) return 0;
  const searchableTags = tags.flatMap((tag) => [tag, ...conceptFragments(tag)]);
  return Math.max(...queryTokensList.map((token) => bestTokenSimilarity(token, searchableTags)), 0);
}

function matchReasons(professor, queryTokensList) {
  const candidates = [...professor.tags, ...professor.tokens];
  return candidates
    .filter((token) => !weakConcepts.has(token))
    .filter((token) => selectedTokens.value.has(token) || bestTokenSimilarity(token, queryTokensList) >= 0.62)
    .filter((token, index, arr) => arr.indexOf(token) === index)
    .sort((a, b) => Number(professor.tags.includes(b)) - Number(professor.tags.includes(a)) || b.length - a.length)
    .slice(0, 8);
}
